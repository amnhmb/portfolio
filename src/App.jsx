...
function SimpleLineChart({ data, title, type, onPointClick }) {
  ...
  return (
    <div className="flex flex-col items-start mt-4 relative w-full overflow-visible">
      ...
      <svg width="100%" viewBox={`0 0 ${w} ${h}`} className="overflow-visible stroke-[#2563eb]">
        {/* Grid lines */}
        ...
        {data.map((val, i) => (
          ...
          >
            ...
            <circle 
              ...
              fill={hoveredIdx === i ? "#2563eb" : "#ECECEC"} 
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="flex items-center justify-between mb-8 p-4 bg-gray-50 rounded-lg border border-gray-100 relative mt-2">
                {semData.gpa !== null && semData.gpa >= 3.5 && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#eff6ff] border border-[#3b82f6] text-[#2563eb] px-3 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-widest shadow-sm whitespace-nowrap">
                    {t('education.deansList')}
                  </div>
                )}
                <div>
                  <p className="text-xs text-gray-500 font-mono uppercase tracking-widest mb-1">GPA</p>
                  <p className="text-3xl font-bold text-[#2563eb]">{semData.gpa !== null ? semData.gpa.toFixed(2) : 'N/A'}</p>
                </div>
                  ...
                  <ul className="space-y-3">
                    {semData.courses.map((course, i) => (
                      <li key={i} className="grid grid-cols-12 items-start text-sm border-b border-gray-100 pb-3 last:border-0">
                        ...
                        <div className="col-span-2 text-center font-bold text-[#2563eb] self-center">
                          {course.grade}
                        </div>
              ...
              <div>
                <div className="mb-10">
                  ...
                  <div className="bg-[#ECECEC] border border-gray-100 rounded-md p-5 inline-block">
                    <div className="text-3xl font-bold text-[#2563eb] mb-1"><AnimatedTextNumber text={t('research.stats.period')} /></div>
